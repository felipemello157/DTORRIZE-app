...
export default function Seguranca() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] pb-24 relative overflow-hidden">
      ...
      <div className="bg-[#13132B]/80 backdrop-blur-md border-b border-white/10 p-6 sticky top-0 z-40">
        <div className="max-w-4xl mx-auto">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-gray-400 hover:text-white font-medium mb-4 transition-colors w-fit p-2 -ml-2 rounded-lg hover:bg-white/5"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-white">Segurança</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6 relative z-10">
        ...
        <motion.div
          ...
          className="bg-[#13132B] border border-white/10 rounded-2xl p-6 shadow-xl"
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-white mb-1">Senha</h3>
                <p className="text-sm text-gray-400">Última alteração: Há 2 meses</p>
              </div>
            </div>
            <button
              onClick={() => setModalSenha(true)}
              className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-xl hover:shadow-lg hover:shadow-blue-500/20 transition-all"
            >
              Alterar Senha
            </button>
          </div>
        ...
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              className={`w-14 h-8 rounded-full transition-all border ${twoFactorEnabled ? "bg-green-500 border-green-400" : "bg-white/10 border-white/20"
                }`}
            >
              <div className={`w-6 h-6 rounded-full bg-white shadow-lg transition-all ${twoFactorEnabled ? "ml-7" : "ml-1"
                }`}></div>
            </button>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-3">
            {sessoes.map((sessao) => (
              <div key={sessao.id} className="border border-white/10 rounded-xl p-4 hover:border-purple-500/50 transition-all bg-[#0a0a1a]">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <Monitor className="w-5 h-5 text-gray-500 mt-1" />
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-semibold text-white">{sessao.device}</p>
                        {sessao.current && (
                          <span className="px-2 py-0.5 bg-green-500/10 text-green-400 border border-green-500/20 text-xs font-bold rounded-full">
                            Atual
                          </span>
                        )}
                      </div>
                      <p className="text-sm text-gray-400">{sessao.location}</p>
        ...
        >
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <h3 className="font-bold text-red-400 mb-1">Zona de Perigo</h3>
              <p className="text-sm text-red-300">Ações irreversíveis</p>
            </div>
          </div>

          <button
            onClick={() => setModalExcluir(true)}
            className="w-full px-4 py-3 bg-[#0a0a1a] border border-red-500/30 text-red-500 font-bold rounded-xl hover:bg-red-500/10 hover:border-red-500/50 transition-all flex items-center justify-center gap-2"
          >
        ...
        {modalSenha && (
          ...
          >
            <motion.div
              ...
              className="bg-[#13132B] border border-white/10 rounded-3xl shadow-2xl max-w-md w-full p-6 relative overflow-hidden"
            >
              ...
              <div className="space-y-4 relative z-10">
                <div>
                  <label className="block text-sm font-semibold text-gray-300 mb-2">Senha Atual</label>
                  <div className="relative">
                    <input
                      type={showPasswords.atual ? "text" : "password"}
                      value={senhaAtual}
                      onChange={(e) => setSenhaAtual(e.target.value)}
                      className="w-full px-4 py-3 pr-12 bg-[#0a0a1a] border border-white/10 text-white rounded-xl focus:border-blue-500 transition-all outline-none"
                    />
                    <button
                      type="button"
              ...
              <div className="flex gap-3 mt-6 relative z-10">
                <button
                  onClick={() => setModalSenha(false)}
                  className="flex-1 py-3 border border-white/10 text-gray-300 font-semibold rounded-xl hover:bg-white/5 transition-all"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="flex-1 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold rounded-xl hover:shadow-lg hover:shadow-blue-500/20 transition-all disabled:opacity-50"
                >
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6 relative z-10">
                ...
                <h2 className="text-xl font-bold text-red-400">Excluir Conta</h2>
              </div>

              <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-4 mb-6 relative z-10">
              ...
              <div className="mb-6 relative z-10">
                <label className="block text-sm font-semibold text-gray-300 mb-2">
                  Digite <span className="text-red-500">EXCLUIR MINHA CONTA</span> para confirmar:
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-3 bg-[#0a0a1a] border border-red-500/30 text-white rounded-xl focus:border-red-500 transition-all outline-none placeholder:text-gray-600"
                />
              </div>
              ...
              <div className="flex gap-3 relative z-10">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-gradient-to-r from-red-500 to-red-600 text-white font-bold rounded-xl hover:shadow-lg hover:shadow-red-500/20 transition-all disabled:opacity-50"
                >
