...
export default function Contratar() {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a1a] flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-gray-400 font-semibold">Carregando dados...</p>
        </div>
      </div>
    );
  }
  ...
  if (sucessoTela) {
    return (
      <div className="min-h-screen bg-[#0a0a1a] flex items-center justify-center p-6 overflow-hidden relative">
        ...
        >
          <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-green-500/20 flex items-center justify-center shadow-2xl border border-green-500/30">
            <CheckCircle2 className="w-14 h-14 text-green-500" />
          </div>

          <h1 className="text-4xl font-black text-white mb-3">Contratação Realizada!</h1>
          ...
          <div className="bg-[#13132B] rounded-3xl border border-white/10 shadow-xl p-6 mb-6">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand-primary to-brand-secondary flex items-center justify-center text-white text-xl font-bold">
                {professional.nome_completo?.charAt(0).toUpperCase()}
              </div>
              <div className="text-left flex-1">
                <p className="font-bold text-white">{professional.nome_completo}</p>
                <p className="text-sm text-gray-400">{professional.especialidade_principal}</p>
              </div>
            </div>
            <div className="bg-[#0a0a1a] border border-white/10 rounded-xl p-4">
              <p className="text-sm text-gray-300">
                <strong className="text-white">Vaga:</strong> {job.titulo}
              </p>
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-brand-primary to-brand-secondary text-white font-bold rounded-2xl shadow-lg shadow-brand-primary/20 hover:shadow-brand-primary/40 hover:scale-[1.02] transition-all"
          >
            Ver Minhas Vagas
          </button>
        </motion.div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] relative overflow-hidden pb-32">
      ...
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-brand-primary/10 rounded-full blur-[120px] opacity-20"></div>
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-brand-secondary/10 rounded-full blur-[120px] opacity-20"></div>

      {/* HEADER */}
      <div className="bg-[#13132B]/80 backdrop-blur-md border-b border-white/5 pt-8 pb-10 px-4 relative z-10">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              ...
              <p className="text-gray-400 mt-1">Confirme os detalhes da contratação</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 mt-8 space-y-6 relative z-10">
        ...
        >
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-bold text-white">Profissional</h3>
            </div>

            <div className="flex items-start gap-4">
              <div className="w-16 h-16 rounded-2xl bg-brand-primary/20 flex items-center justify-center text-brand-primary text-2xl font-bold flex-shrink-0 border border-brand-primary/30">
                {professional.nome_completo?.charAt(0).toUpperCase()}
              </div>

              <div className="flex-1">
                <h4 className="text-xl font-bold text-white mb-1">{professional.nome_completo}</h4>
                <p className="text-gray-400 mb-2">{professional.especialidade_principal}</p>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        className={`w-4 h-4 ${star <= Math.round(mediaAvaliacoes) ? "fill-brand-orange text-brand-orange" : "text-gray-700"}`}
                      />
                    ))}
                  </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-bold text-white">Vaga</h3>
            </div>

            <div className="space-y-3">
              <div>
                <h4 className="text-lg font-bold text-white">{job.titulo}</h4>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex items-start gap-2 p-3 bg-[#0a0a1a] border border-white/5 rounded-xl">
                  <Briefcase className="w-5 h-5 text-gray-400 mt-0.5" />
                  <div>
                    <p className="text-xs text-gray-500">Tipo</p>
                    <p className="font-semibold text-gray-200">{tipoVagaConfig[job.tipo_vaga]}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2 p-3 bg-[#0a0a1a] border border-white/5 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-200">{job.cidade} - {job.uf}</p>
                  </div>
                </div>
                ...
                {job.dias_semana && job.dias_semana.length > 0 && (
                  <div className="flex items-start gap-2 p-3 bg-[#0a0a1a] border border-white/5 rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-200">{job.dias_semana.join(", ")}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-start gap-2 p-3 bg-[#0a0a1a] border border-white/5 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-200">{job.horario_inicio} - {job.horario_fim}</p>
                  </div>
                </div>

                {job.valor_proposto && job.tipo_remuneracao !== "A_COMBINAR" && (
                  <div className="flex items-start gap-2 p-3 bg-green-500/10 border border-green-500/20 rounded-xl md:col-span-2">
                    <DollarSign className="w-5 h-5 text-green-500 mt-0.5" />
                    <div>
                      <p className="text-xs text-green-400">Valor Proposto</p>
        ...
        <motion.div
          ...
          className="bg-[#13132B] rounded-3xl border border-white/10 shadow-xl p-6"
        >
          <h3 className="font-bold text-white mb-4">Confirmação</h3>

          <div className="space-y-4">
            <label className="flex items-start gap-3 p-4 rounded-xl border border-white/10 hover:border-brand-primary/50 transition-all cursor-pointer bg-[#0a0a1a]">
              <input
                type="checkbox"
                checked={confirmoContato}
                onChange={(e) => setConfirmoContato(e.target.checked)}
                className="w-5 h-5 mt-0.5 accent-brand-primary cursor-pointer bg-transparent border-white/20"
              />
        ...
        <motion.div
          ...
          className="bg-brand-primary/10 border border-brand-primary/20 rounded-2xl p-5"
        >
          <div className="flex gap-3">
            ...
            <div>
              <p className="font-semibold text-brand-primary mb-1">Sistema de Avaliação</p>
              <p className="text-sm text-gray-300">
      ...
      {/* FOOTER FIXO */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#13132B]/90 backdrop-blur-xl border-t border-white/10 p-4 shadow-2xl z-40">
        <div className="max-w-3xl mx-auto flex gap-3">
          <button
            onClick={() => navigate(-1)}
            disabled={contratarMutation.isPending}
            className="flex-1 py-4 border border-white/20 text-gray-300 font-bold rounded-2xl hover:bg-white/5 transition-all disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
