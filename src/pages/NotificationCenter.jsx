...
function NotificationCard({ notification, index, onClick, config }) {
  ...
  return (
    <motion.div
      ...
      className={`bg-[#13132B] rounded-3xl p-4 shadow-lg border relative overflow-hidden cursor-pointer hover:bg-[#1a1a3a] hover:scale-[1.01] transition-all group ${!notification.lida
    ...
    >
      ...
      {!notification.lida && (
        <div className="absolute inset-0 bg-gradient-to-r from-brand-primary/10 to-transparent opacity-60 pointer-events-none"></div>
      )}
      ...
      <div className="flex items-start gap-4 relative z-10">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-bold text-white flex items-center gap-2 mb-1 group-hover:text-brand-primary transition-colors">
            {notification.titulo}
            {!notification.lida && (
              <span className="w-2 h-2 bg-brand-primary rounded-full animate-pulse flex-shrink-0"></span>
            )}
          </h4>
        ...
        {notification.acao_destino?.destino && (
          <button className="ml-auto text-brand-primary font-bold text-sm hover:text-brand-secondary flex items-center gap-1 flex-shrink-0 px-3 py-2 bg-brand-primary/10 rounded-xl hover:bg-brand-primary/20 transition-all">
            Ver
...
export default function NotificationCenter() {
  ...
  const getNotificationConfig = (tipo) => {
    const configs = {
      MATCH_PERFEITO: { icon: Zap, gradient: "from-brand-primary to-brand-secondary", emoji: "⚡" },
      CANDIDATURA_ACEITA: { icon: CheckCircle2, gradient: "from-green-500 to-emerald-600", emoji: "✅" },
      CANDIDATURA_REJEITADA: { icon: XCircle, gradient: "from-red-500 to-rose-600", emoji: "❌" },
      NOVA_CANDIDATURA: { icon: UserPlus, gradient: "from-blue-500 to-indigo-600", emoji: "👤" },
      AVALIACAO_RECEBIDA: { icon: Star, gradient: "from-yellow-400 to-orange-500", emoji: "⭐" },
      CONTRATO_CRIADO: { icon: FileText, gradient: "from-green-500 to-emerald-600", emoji: "📄" },
      SISTEMA: { icon: Bell, gradient: "from-gray-500 to-gray-700", emoji: "🔔" },
      // Tipos antigos
      SUPER_JOBS: { icon: Zap, gradient: "from-brand-primary to-brand-secondary", emoji: "⚡" },
      JOBS_SEMELHANTE: { icon: TrendingUp, gradient: "from-purple-500 to-indigo-500", emoji: "🎯" },
      VAGA_NORMAL: { icon: Briefcase, gradient: "from-blue-500 to-cyan-500", emoji: "💼" },
      MENSAGEM_CHAT: { icon: MessageCircle, gradient: "from-brand-primary to-pink-600", emoji: "💬" },
      STATUS_APROVADO: { icon: CheckCircle2, gradient: "from-green-500 to-emerald-600", emoji: "✅" },
      STATUS_REPROVADO: { icon: AlertCircle, gradient: "from-red-500 to-rose-600", emoji: "❌" },
      MATCH_VISUALIZADO: { icon: Star, gradient: "from-yellow-400 to-orange-500", emoji: "👁️" },
      MATCH_CONTATADO: { icon: Award, gradient: "from-purple-500 to-violet-600", emoji: "💰" },
      VAGA_PREENCHIDA: { icon: CheckCircle2, gradient: "from-green-500 to-emerald-600", emoji: "🎉" },
      RECEBEU_AVALIACAO: { icon: Star, gradient: "from-yellow-400 to-orange-500", emoji: "⭐" },
    };
    return configs[tipo] || { icon: Bell, gradient: "from-gray-500 to-gray-700", emoji: "🔔" };
  };
    ...
    return (
      <div className="min-h-screen bg-[#0a0a1a] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 font-semibold">Carregando notificações...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] text-white">
      {/* Header Gradiente */}
      <div className="bg-[#13132B] border-b border-white/5 pt-6 pb-8 px-4">
        <div className="max-w-4xl mx-auto flex justify-between items-start">
          {/* Lado Esquerdo */}
          <div>
            <div className="w-12 h-12 rounded-2xl bg-brand-primary/20 flex items-center justify-center mb-2">
              <Bell className="w-6 h-6 text-brand-primary" />
            </div>
            <h1 className="text-2xl md:text-3xl font-black text-white mb-2">Notificações</h1>
            {naoLidas > 0 && (
              <div className="inline-flex items-center gap-1 bg-brand-primary/20 border border-brand-primary/30 text-brand-primary text-sm font-semibold px-3 py-1 rounded-full">
      ...
      {/* Tabs/Filtros */}
      <div className="bg-[#0a0a1a]/80 backdrop-blur-md sticky top-0 z-20 border-b border-white/5">
          ...
          <div className="flex overflow-x-auto gap-2 p-4 no-scrollbar">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-5 py-2.5 font-bold rounded-full whitespace-nowrap transition-all border ${activeTab === tab.value
                    ? "bg-brand-primary text-white border-brand-primary shadow-[0_0_15px_rgba(255,102,0,0.3)]"
      ...
      <div className="max-w-4xl mx-auto p-4 pb-24">
        {filteredNotifications.length === 0 ? (
          <motion.div
            ...
            className="bg-[#13132B] rounded-3xl p-12 text-center border border-white/5"
          >
            ...
            <p className="text-gray-600">Quando houver novidades, você verá aqui</p>
          </motion.div>
        ) : (
          <>
            {/* HOJE */}
      ...
      {naoLidas > 0 && (
        <motion.div
          ...
          className="fixed bottom-24 md:bottom-6 left-4 right-4 max-w-4xl mx-auto z-30"
        >
          <button
            onClick={() => marcarTodasComoLidasMutation.mutate()}
            disabled={marcarTodasComoLidasMutation.isPending}
            className="w-full py-4 bg-brand-primary text-white font-bold rounded-2xl shadow-[0_0_20px_rgba(255,102,0,0.4)] hover:shadow-[0_0_30px_rgba(255,102,0,0.6)] hover:scale-[1.02] transition-all disabled:opacity-50 flex items-center justify-center gap-2 border border-brand-primary/50"
          >
