  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] pb-24 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 relative z-10">
        {/* Header */}
        <div className="mb-6">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-400 hover:text-white font-medium mb-4 transition-colors">
            <ChevronLeft className="w-5 h-5" />
            Voltar
          </button>

          <div className="bg-gradient-to-r from-brand-primary/20 to-purple-500/20 border border-white/10 rounded-3xl p-6 backdrop-blur-xl relative overflow-hidden">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-brand-primary to-purple-600 flex items-center justify-center shadow-lg shadow-purple-900/50">
                <User className="w-8 h-8 text-white" />
              </div>
              <div>
                <h1 className="text-3xl font-black text-white">Criar Anúncio</h1>
                <p className="text-gray-400 text-sm">Mostre que você está disponível</p>
              </div>
            </div>

            <div className="h-3 bg-white/5 rounded-full overflow-hidden backdrop-blur">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${(etapa / 3) * 100}%` }}
                className="h-full bg-gradient-to-r from-brand-primary to-purple-500 shadow-lg shadow-brand-primary/50"
              />
            </div>
            <p className="text-xs text-gray-400 mt-2 font-semibold">Etapa {etapa} de 3 • {Math.round((etapa / 3) * 100)}% completo</p>
          </div>
        </div>
        ...
        <motion.div
          ...
          className="bg-[#13132B]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8"
        >
          {etapa === 1 && (
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all"
                />
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Sobre Você *</label>
                <textarea
                  value={formData.descricao}
                  onChange={(e) => handleChange("descricao", e.target.value)}
                  placeholder="Conte sobre sua experiência, habilidades e o que você busca..."
                  className="w-full min-h-[120px] px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none resize-none transition-all"
                  maxLength={800}
                />
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Tipo de Vaga de Interesse *</label>
                <div className="grid grid-cols-2 gap-3">
                  {(professional?.tipo_profissional === "DENTISTA"
                    ...
                    <motion.button
                      ...
                      className={`py-3 px-4 rounded-xl font-bold transition-all text-sm ${formData.tipos_vaga_interesse.includes(tipo)
                          ? "bg-gradient-to-r from-brand-primary to-purple-600 text-white shadow-lg shadow-brand-primary/20"
                          : "bg-white/5 border border-white/10 text-gray-400 hover:border-brand-primary/50 hover:bg-white/10"
                        }`}
                    >
              ...
              <motion.label
                ...
                className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl cursor-pointer hover:bg-white/10 transition-all"
              >
                <input
                  type="checkbox"
                  checked={formData.aceita_freelance}
                  onChange={(e) => handleChange("aceita_freelance", e.target.checked)}
                  className="w-5 h-5 accent-brand-primary rounded bg-white/10 border-white/20"
                />
                <span className="text-sm text-gray-300 font-semibold">⚡ Aceito trabalho freelance/substituição temporária</span>
              </motion.label>
            </div>
          )}
          ...
          {etapa === 2 && (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Cidades de Interesse * (até 5)</label>
                <div className="flex gap-2 mb-3">
                  <select
                    value={formData.uf}
                    onChange={(e) => handleChange("uf", e.target.value)}
                    className="w-24 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:border-brand-primary outline-none [&>option]:bg-[#13132B]"
                  >
                  ...
                  <motion.button
                    ...
                    className="px-5 py-2 bg-brand-primary text-white font-bold rounded-xl shadow-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-brand-primary/90"
                  >
                    + Add
                  </motion.button>
                </div>
                ...
                {formData.cidades_interesse.length > 0 && (
                  <div className="flex flex-wrap gap-2 p-3 bg-white/5 rounded-xl border border-white/10">
                    {formData.cidades_interesse.map((cidade) => (
                      <span key={cidade} className="inline-flex items-center gap-1 px-3 py-1 bg-brand-primary/20 border border-brand-primary/30 text-brand-primary rounded-full text-sm font-medium">
                        {cidade}
                        <button onClick={() => removerCidade(cidade)} className="hover:text-white transition-colors">×</button>
                      </span>
                    ))}
                  </div>
                )}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Dias Disponíveis *</label>
                <div className="grid grid-cols-4 gap-2">
                  {["SEG", "TER", "QUA", "QUI", "SEX", "SAB", "DOM", "INTEGRAL"].map((dia) => (
                    <motion.button
                      ...
                      className={`py-2.5 rounded-xl font-bold text-xs transition-all ${formData.dias_semana_disponiveis.includes(dia)
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:border-brand-primary outline-none [color-scheme:dark]"
                  />
                </div>
                <div>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Quando Pode Começar? *</label>
                <select
          ...
          {etapa === 3 && (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Formas de Remuneração Aceitas *</label>
                <div className="grid grid-cols-2 gap-3">
                  {["FIXO", "DIARIA", "PORCENTAGEM", "A_COMBINAR"].map((tipo) => (
                    <motion.button
                      ...
                      className={`py-3 px-4 rounded-xl font-bold text-sm transition-all ${formData.formas_remuneracao_aceitas.includes(tipo)
                          ? "bg-green-600 text-white shadow-lg shadow-green-900/20"
                          : "bg-white/5 border border-white/10 text-gray-400 hover:border-green-500/50 hover:bg-white/10"
                        }`}
                    >
                      {tipo === "FIXO" && "💰 Fixo (CLT)"}
                      {tipo === "DIARIA" && "📅 Diária"}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Pretensão Salarial (opcional)</label>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="0,00"
                        className="w-full pl-10 pr-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:border-brand-primary outline-none"
                      />
                    </div>
                  </div>
                  <div>
              ...
              <motion.div
                ...
                className="bg-brand-primary/10 rounded-2xl p-5 border border-brand-primary/20"
              >
                <h3 className="text-base font-black text-white mb-3 flex items-center gap-2">
                  📋 Resumo do Anúncio
                </h3>
                <div className="space-y-2 text-sm bg-black/20 rounded-xl p-4 border border-white/5">
                  <p className="flex items-start gap-2"><strong className="text-white min-w-[100px]">Título:</strong> <span className="text-gray-300">{formData.titulo}</span></p>
                  <p className="flex items-start gap-2"><strong className="text-white min-w-[100px]">Tipos vaga:</strong> <span className="text-gray-300">{formData.tipos_vaga_interesse.join(", ")}</span></p>
                  <p className="flex items-start gap-2"><strong className="text-white min-w-[100px]">Cidades:</strong> <span className="text-gray-300">{formData.cidades_interesse.slice(0, 2).join(", ")}{formData.cidades_interesse.length > 2 && "..."}</span></p>
                  <p className="flex items-start gap-2"><strong className="text-white min-w-[100px]">Início:</strong> <span className="text-gray-300">{formData.disponibilidade_inicio}</span></p>
                  <p className="flex items-start gap-2"><strong className="text-white min-w-[100px]">Pagamento:</strong> <span className="text-gray-300">{formData.formas_remuneracao_aceitas.join(", ")}</span></p>
                </div>
              </motion.div>
            </div>
          )}
          ...
          <div className="flex gap-3 mt-6">
            ...
            {etapa < 3 ? (
              <motion.button
                ...
                className="flex-1 py-4 bg-gradient-to-r from-brand-primary to-purple-600 text-white font-black rounded-xl shadow-lg hover:shadow-xl shadow-brand-primary/20 flex items-center justify-center gap-2"
              >
                Continuar
              ...
              <motion.button
                ...
                onClick={publicar}
                disabled={loading}
                className="flex-1 py-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-black rounded-xl shadow-xl shadow-green-900/20 hover:shadow-2xl disabled:opacity-50 flex items-center justify-center gap-2"
              >
