    ...
    <div className="min-h-screen bg-[#0a0a1a] relative overflow-hidden pb-24">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 relative z-10">
        ...
        <motion.div
          ...
          className="bg-[#13132B] border border-white/10 rounded-3xl p-6 mb-8 shadow-xl"
        >
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 bg-white/5 border border-white/10 rounded-2xl text-white focus:border-brand-primary outline-none transition-all placeholder:text-gray-600"
            />
          </div>
        </motion.div>
        ...
        <AnimatePresence>
          {faqsFiltrados && faqsFiltrados.length > 0 && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="bg-[#13132B] border border-white/10 rounded-3xl p-6 mb-6"
            >
              ...
              <div className="space-y-4">
                {faqsFiltrados.map((faq, idx) => (
                  <div key={idx} className="border-b border-white/5 last:border-0 pb-4 last:pb-0">
                    <p className="text-xs text-brand-primary font-bold mb-1">{faq.categoriaTitulo}</p>
                    <button
                      onClick={() => handlePerguntaClick(faq.categoriaKey, faq.index)}
                      className="w-full text-left"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <h3 className="font-bold text-white hover:text-brand-primary transition-colors">{faq.pergunta}</h3>
                    ...
                    {perguntaAberta === `${faq.categoriaKey}-${faq.index}` && (
                      <motion.div
                        ...
                        className="mt-3 text-gray-400 leading-relaxed bg-white/5 p-4 rounded-xl"
                      >
                        {faq.resposta}
                      </motion.div>
                    )}
          ...
          {faqsFiltrados && faqsFiltrados.length === 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-[#13132B] border border-white/10 rounded-3xl p-12 text-center mb-6"
            >
              <Search className="w-16 h-16 text-gray-600 mx-auto mb-4" />
              <p className="text-gray-400 font-semibold">Nenhum resultado encontrado para "{busca}"</p>
        ...
        {!busca.trim() && (
          <>
            <div className="mb-8">
              <h2 className="text-xl font-black text-white mb-6">Categorias de Ajuda</h2>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {Object.entries(faqData).map(([key, categoria], index) => {
                  const Icon = categoria.icone;
                  const isSelected = categoriaAberta === key;
                  return (
                    <motion.button
                      key={key}
                      initial={{ opacity: 0, y: 20 }}
                      ...
                      className={`relative overflow-hidden bg-[#13132B] border ${isSelected ? "border-brand-primary" : "border-white/10"} rounded-3xl p-6 hover:bg-white/5 transition-all text-left group`}
                    >
                      <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${categoria.cor} flex items-center justify-center text-white mb-3 shadow-lg group-hover:scale-110 transition-transform`}>
                        <Icon className="w-6 h-6" />
                      </div>
                      <h3 className={`font-black text-sm mb-1 ${isSelected ? "text-brand-primary" : "text-white"}`}>{categoria.titulo}</h3>
                      <p className="text-xs text-gray-500">{categoria.perguntas.length} perguntas</p>
              ...
              {categoriaAberta && (
                ...
                >
                  <div className="flex items-center gap-3 mb-6 pb-6 border-b border-white/5">
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${faqData[categoriaAberta].cor} flex items-center justify-center text-white`}>
                  ...
                  <div className="space-y-4">
                    {faqData[categoriaAberta].perguntas.map((faq, index) => (
                      <div key={index} className="border-b border-white/5 last:border-0 pb-4 last:pb-0">
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <h3 className="font-bold text-white pr-4">{faq.pergunta}</h3>
                          ...
                          <AnimatePresence>
                            {perguntaAberta === `${categoriaAberta}-${index}` && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, height: "auto" }}
                                exit={{ opacity: 0, height: 0 }}
                                className="mt-3 text-gray-400 leading-relaxed border-t border-white/10 pt-3"
                              >
                                {faq.resposta}
                              </motion.div>
                            )}
        ...
        <motion.div
          ...
          className="bg-[#13132B] border border-white/10 rounded-3xl p-8 mb-6 relative overflow-hidden"
        >
          <div className="relative z-10">
            <h2 className="text-xl font-black text-white mb-2">Ainda precisa de ajuda?</h2>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 px-6 py-3 border border-white/20 text-white font-bold rounded-2xl hover:bg-white/10 transition-all bg-white/5"
              >
        ...
        <motion.div
          ...
          className="bg-[#13132B] border border-white/10 rounded-3xl p-6"
        >
          <h2 className="text-xl font-black text-white mb-4">Links Úteis</h2>
          <div className="space-y-3">
            <a
              href="https://newjobs.com.br/termos"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-between px-4 py-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all group"
            >
              <span className="font-semibold text-gray-300 group-hover:text-white transition-colors">Termos de Uso</span>
              <ExternalLink className="w-5 h-5 text-gray-500 group-hover:text-brand-primary transition-colors" />
            </a>
            <a
              ...
              className="flex items-center justify-between px-4 py-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all group"
            >
              <span className="font-semibold text-gray-300 group-hover:text-white transition-colors">Política de Privacidade</span>
              <ExternalLink className="w-5 h-5 text-gray-500 group-hover:text-brand-primary transition-colors" />
            </a>
            <a
              ...
              className="flex items-center justify-between px-4 py-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all group"
            >
              <span className="font-semibold text-gray-300 group-hover:text-white transition-colors">Sobre o NEW JOBS</span>
              <ExternalLink className="w-5 h-5 text-gray-500 group-hover:text-brand-primary transition-colors" />
            </a>
          </div>
        </motion.div>
