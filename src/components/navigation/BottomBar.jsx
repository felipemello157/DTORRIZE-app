...
export default function BottomBar() {
  ...
  useEffect(() => {
    const detectUserType = async () => {
      try {
        ...
        // SE FOR MOCK USER (DEV), FORÇAR TIPO PROFISSIONAL
        if (user.mock) {
          setUserType("PROFISSIONAL");
          localStorage.setItem('doutorizze_user_type', "PROFISSIONAL");
          return;
        }
        ...
        // Erro silencioso - mas se for localhost, forçar visualização
        if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
          console.warn("⚠️ [DEV] BottomBar: Auth falhou. Forçando menu PROFISSIONAL para debug.");
          setUserType("PROFISSIONAL");
        }
      }
    };
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-2xl pb-safe">
        ...
        {botoes.map((botao, index) => {
          ...
          if (botao.isCenter) {
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center flex-1 h-full relative transition-all hover:scale-105 ${centralActive ? "-mt-4" : ""
                  }`}
              >
                ...
                {/* Texto */}
                <span className={`text-xs ${centralActive
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center flex-1 h-full relative transition-all hover:scale-105 ${active ? "-mt-4" : ""
                }`}
            >
              ...
              {/* Texto */}
              <span className={`text-xs ${active ? "text-purple-500 font-bold mt-1" : "text-gray-400"
                }`}>
                {botao.label}
              </span>
            </button>
